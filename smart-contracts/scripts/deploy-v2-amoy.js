 ...
 * Polygon Amoy Testnet (Mumbai replacement) Deployment
 *
 * Deploys:
...
async function main() {
    console.log("🚀 La Tanda Chain - Tokenomics V2.0 Deployment to Polygon Amoy Testnet (Mumbai replacement)");
    console.log("================================================================\n");

    const [deployer] = await hre.ethers.getSigners();
    console.log("📍 Deploying from address:", deployer.address);
    console.log("💰 Account balance:", hre.ethers.utils.formatEther(await deployer.getBalance()), "POL\n");
    ...
    if (network.chainId !== 80002) {
        console.error("❌ ERROR: Not connected to Amoy testnet (expected chainId 80002)");
        console.error("   Current chainId:", network.chainId);
        process.exit(1);
    }
    console.log("✅ Amoy testnet confirmed\n");
    ...
    try {
        ...
        const filename = `amoy-v2-${timestamp}.json`;
        ...
        console.log(`   LTDToken: https://amoy.polygonscan.com/address/${ltdToken.address}`);
        console.log(`   RoyalVesting: https://amoy.polygonscan.com/address/${royalVesting.address}`);
        console.log(`   FutureReserve: https://amoy.polygonscan.com/address/${futureReserve.address}`);
        console.log();

        console.log("🔍 Next Steps:");
        console.log("   1. Verify contracts on Polygonscan:");
        console.log(`      npx hardhat verify --network amoy ${ltdToken.address}`);
        console.log(`      npx hardhat verify --network amoy ${royalVesting.address} "${ltdToken.address}" "${beneficiaryAddress}"`);
        console.log(`      npx hardhat verify --network amoy ${futureReserve.address} "${ltdToken.address}"`);
        console.log();
